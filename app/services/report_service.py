    ...
    payslips = (
        await db.execute(
            select(Payslip).where(
                Payslip.cycle_id == cycle.id, Payslip.company_id == company_id
            )
        )
    ).scalars().all()
    employees = {
...
async def payroll_summary_records(
...
def salary_register_pdf(
    company_name: str, cycle: PayrollCycle, records: list[dict[str, Any]]
) -> bytes:
    subtitle = (
...
def payroll_summary_pdf(company_name: str, records: list[dict[str, Any]]) -> bytes:
    ...
    return _table_pdf(
        "Payroll Summary", f"{company_name}  ·  {len(records)} cycle(s)", pdf_columns, records
    )
