...
async def list_types(
    ...
    q = select(LeaveType).where(
        LeaveType.company_id == company_id, LeaveType.deleted_at.is_(None)
    )
    if active_only:
...
async def create_type(db: AsyncSession, company_id: uuid.UUID, payload) -> LeaveType:
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="A leave type with this code already exists.",
        )
    except Exception:
        await db.rollback()
        raise
...
async def _get_type(
    db: AsyncSession, company_id: uuid.UUID, leave_type_id: uuid.UUID
) -> LeaveType:
    lt = (
        await db.execute(
...
async def list_balances(
    ...
    *,
    ...
    from app.models.employee import Employee  # local import avoids a cycle
...
async def create_request(
    ...
    if not working:
        raise HTTPException(
            status_code=400,
            detail="The selected range has no working days (only weekly-offs/holidays).",
        )
...
async def list_requests(
    ...
    *,
    ...
    q = select(LeaveRequest).where(
        LeaveRequest.company_id == company_id, LeaveRequest.deleted_at.is_(None)
    )
    if status_filter:
...
async def approve_request(
    ...
    if lt.is_paid:
        bal = await ensure_balance(
            db, company_id, req.employee_id, lt, DEFAULT_FINANCIAL_YEAR,
...
async def reject_request(db, company_id, request_id, actor_id=None, note=None) -> LeaveRequest:
    return await _decide_no_balance(
        db, company_id, request_id, LeaveStatus.REJECTED.value, actor_id, note
    )


async def cancel_request(db, company_id, request_id, actor_id=None, note=None) -> LeaveRequest:
    return await _decide_no_balance(
        db, company_id, request_id, LeaveStatus.CANCELLED.value, actor_id, note
    )
...
async def get_approved_leave_days(
    ...
    paid = {
        t.id: t.is_paid
        for t in (
            await db.execute(select(LeaveType).where(LeaveType.id.in_(type_ids)))
        ).scalars().all()
    }
    ...
    for r in reqs:
        ...
        full = (
            ...
            if paid.get(r.leave_type_id, True)
            else DayStatus.UNPAID_LEAVE.value
        )
        day = lo
        while day <= hi:
            out[day] = full
