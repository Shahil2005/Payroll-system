...
from app.constants import DEFAULT_WORKING_DAYS, WEEKDAY_CODES
from app.models.calendar import Holiday
from app.models.company import Company
from app.schema.settings import WorkCalendarConfig


async def load_calendar_config(
    db: AsyncSession, company_id: uuid.UUID
) -> WorkCalendarConfig:
    """The company's work-calendar config (defaults when unset)."""
    raw = (
        await db.execute(
            select(Company.statutory_settings).where(Company.id == company_id)
        )
    ).scalar_one_or_none()
...
async def working_days_in_period(
    db: AsyncSession, company_id: uuid.UUID, start: date, end: date
) -> Decimal:
...
async def create_holiday(
    db: AsyncSession, company_id: uuid.UUID, holiday_date: date, name: str
) -> Holiday:
...
async def delete_holiday(
    db: AsyncSession, company_id: uuid.UUID, holiday_id: uuid.UUID
) -> None:
    holiday = (
        await db.execute(
