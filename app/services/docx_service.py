...
_CURRENCY_UNITS = {
    "INR": ("Rupees", "Paise"), "NPR": ("Rupees", "Paisa"),
    "LKR": ("Rupees", "Cents"), "PKR": ("Rupees", "Paisa"),
    "BDT": ("Taka", "Poisha"),
    "USD": ("Dollars", "Cents"), "AUD": ("Dollars", "Cents"),
    "CAD": ("Dollars", "Cents"), "SGD": ("Dollars", "Cents"),
    "HKD": ("Dollars", "Cents"), "NZD": ("Dollars", "Cents"),
    "EUR": ("Euros", "Cents"), "GBP": ("Pounds", "Pence"),
    "AED": ("Dirhams", "Fils"), "SAR": ("Riyals", "Halalas"),
    "QAR": ("Riyals", "Dirhams"), "JPY": ("Yen", "Sen"),
    "CNY": ("Yuan", "Fen"), "CHF": ("Francs", "Centimes"),
    "ZAR": ("Rand", "Cents"), "MYR": ("Ringgit", "Sen"),
    "THB": ("Baht", "Satang"), "IDR": ("Rupiah", "Sen"),
    "PHP": ("Pesos", "Centavos"),
}
...
_LOGO_PHRASES = {
    "logo", "company logo", "logo here", "company logo here", "your logo",
    "company logo name", "logo image", "company name logo", "insert logo",
    "insert company logo", "logo goes here",
}
...
_FIELDS: list[tuple[str, str, str, list[str]]] = [
    ...
    ("Employee", "employee.code", "Employee code / ID",
        ["employee code", "employee id", "emp id", "emp code", "employee no", "staff id", "emp no", "employee number"]),
    ("Employee", "employee.designation", "Designation",
        ["designation", "title", "job title", "role"]),
    ("Employee", "employee.department", "Department", ["department", "dept"]),
    ("Employee", "employee.location", "Location / Branch",
        ["location", "work location", "branch", "office"]),
    ("Employee", "employee.email", "Email", ["email", "email id", "e mail"]),
    ("Employee", "employee.bank_account_no", "Bank account no.",
        ["bank account", "account no", "account number", "a c no", "bank a c no", "bank account number", "bank a c"]),
    ("Employee", "employee.pan", "Employee PAN", ["pan", "pan no", "pan number"]),
    ("Employee", "employee.uan", "UAN", ["uan", "uan no", "uan number"]),
    ("Employee", "employee.esic", "ESIC number",
        ["esic", "esic no", "esi number", "esic number"]),
    ("Employee", "employee.state", "Employee state", ["state"]),
    ("Employee", "employee.date_of_joining", "Date of joining",
        ["date of joining", "doj", "joining date"]),
    # --- Pay period ---
    ("Pay period", "cycle_name", "Pay period / month",
    ...
    ("Pay period", "pay_date", "Pay date",
        ["pay date", "payment date", "date of payment", "paid on"]),
    ("Pay period", "ref", "Reference no.",
        ["ref", "reference", "payslip no", "reference no", "slip no"]),
    # --- Attendance ---
    ("Attendance", "working_days", "Working days",
        ["working days", "total days", "days in month"]),
    ("Attendance", "paid_days", "Paid days", ["paid days", "days paid", "days worked"]),
    ("Attendance", "lop_days", "Loss-of-pay days",
        ["lop", "lop days", "loss of pay", "absent days"]),
    ...
    ("Earnings", "amount.CONVEYANCE", "Conveyance",
    ...
    ("Earnings", "amount.SPECIAL", "Special allowance",
        ["special allowance", "special", "other allowance", "spl allowance"]),
    # --- Deductions (component amounts) ---
    ("Deductions", "amount.PF", "Provident fund (PF)",
        ["pf", "provident fund", "epf", "employee pf", "pf contribution"]),
    ("Deductions", "amount.ESI", "ESI", ["employee esi", "esi contribution"]),
    ("Deductions", "amount.PT", "Professional tax",
        ["professional tax", "pt", "p tax", "prof tax"]),
    ("Deductions", "amount.TDS", "Income tax (TDS)",
        ["tds", "income tax", "tax deducted", "i tax"]),
    # --- Totals ---
    ("Totals", "gross", "Gross earnings",
        ["gross", "gross earnings", "gross salary", "total earnings", "total gross"]),
    ("Totals", "total_deductions", "Total deductions",
        ["total deductions", "deductions", "total deduction"]),
    ("Totals", "net", "Net pay",
        ["net pay", "net salary", "net payable", "take home", "amount payable", "net amount", "net"]),
    ("Totals", "net_in_words", "Net pay (in words)",
        ["net pay in words", "amount in words", "net amount in words", "in words",
         "rupees in words", "net salary in words", "net payable in words",
         "amount payable in words", "amount in word", "rupees", "in words only"]),
    ...
    ("Company", "company_name", "Company name",
        ["company name", "organisation", "organization", "company"]),
    ...
    ("Company", "company.address", "Company address",
        ["company address", "address", "registered address"]),
...
FIELD_CATALOG: list[dict[str, str]] = [
    {"group": g, "key": k, "label": label} for g, k, label, _ in _FIELDS
]
...
def scan_docx_fields(
    data: bytes, extra_synonyms: dict[str, str] | None = None
) -> list[dict[str, Any]]:
...
def _run_image_html(run) -> str:
    ...
    for blip in blips:
        ...
        try:
            ...
            out.append(
                f'<img src="data:{ctype};base64,{b64}"{dim} style="max-height:70px;"/>'
            )
        except Exception:
            pass
    return "".join(out)
...
def _table_html(table) -> str:
    ...
    for row in table.rows:
        ...
        for cell in row.cells:
            ...
            cells_html.append(
                f'<td style="border:1px solid #d1d5db;padding:4px 8px;">{inner}</td>'
            )
        rows_html.append("<tr>" + "".join(cells_html) + "</tr>")
    return (
        '<table border="1" width="100%" '
        'style="width:100%;border-collapse:collapse;margin:6px 0;">'
        + "".join(rows_html)
        + "</table>"
    )
