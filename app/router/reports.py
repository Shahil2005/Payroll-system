...
from app.services import payroll_service, report_service
...
async def salary_register(
    ...
    if format == "csv":
        content = report_service.records_to_csv(
            report_service.SALARY_REGISTER_COLUMNS, records
        )
    else:
        company_name = await _company_name(db, company_id)
        content = await run_in_threadpool(
            report_service.salary_register_pdf, company_name, cycle, records
        )
...
async def payroll_summary(
    ...
    if format == "csv":
        content = report_service.records_to_csv(
            report_service.PAYROLL_SUMMARY_COLUMNS, records
        )
    else:
        company_name = await _company_name(db, company_id)
        content = await run_in_threadpool(
            report_service.payroll_summary_pdf, company_name, records
        )
