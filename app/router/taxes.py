...
from app.constants import Permission, PayrollCycleStatus
...
from app.models.taxes import EmployeeTaxProfile, TdsChallan
from app.schema.taxes import (
    ChallanCreate,
    ChallanOut,
    TaxProfileOut,
    TaxProfileUpsert,
    ...
    challans = (
        await db.execute(
            select(TdsChallan).where(
                TdsChallan.company_id == company_id, TdsChallan.deleted_at.is_(None)
            )
        )
    ).scalars().all()
