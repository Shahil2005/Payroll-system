...
def _payslip_out(company: Company) -> PayslipSettingsOut:
    ...
    return PayslipSettingsOut(
        **settings.model_dump(),
        ...
        doc_mapped=company.payslip_doc_original is not None
        and bool(company.payslip_doc_mapping),
...
async def _company_components(
    ...
    for code, label in sorted(seen.items()):
        ...
        if token not in static_keys:
            fields.append(
                PayslipFieldOption(
                    group="Your salary components", key=token, label=f"{label} ({code})"
                )
            )
...
def _scan_to_out(
    filename: str,
    data: bytes,
    extra_fields: list[PayslipFieldOption],
    synonyms: dict[str, str],
) -> PayslipDocScanOut:
    slots = docx_service.scan_docx_fields(data, extra_synonyms=synonyms)
    return PayslipDocScanOut(
        filename=filename,
        slots=slots,
        fields=_field_catalog(extra_fields),
    )
...
async def scan_payslip_document(
    ...
    try:
        out = _scan_to_out(
            (file.filename or "payslip-template.docx")[:255], data, extra_fields, synonyms
        )
    except Exception:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
...
async def apply_payslip_mapping(
    ...
    if not clean:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="Map at least one field before saving.",
        )
    ...
    scan_slots = docx_service.scan_docx_fields(
        company.payslip_doc_original, extra_synonyms=synonyms
    )
    label_by_index = {s["index"]: s["label"] for s in scan_slots}
    company.payslip_doc_mapping = {
        str(i): {"token": tok, "label": label_by_index.get(i, "")}
        for i, tok in by_index.items()
    }
