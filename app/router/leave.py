...
from app.constants import DEFAULT_FINANCIAL_YEAR, Permission
...
async def list_balances(
    ...
    rows = await leave_service.list_balances(
        db, company_id, financial_year, employee_id=employee_id
    )
...
async def approve_leave_request(
    ...
    req = await leave_service.approve_request(
        db, company_id, request_id, current_user.id, payload.note
    )
...
async def reject_leave_request(
    ...
    req = await leave_service.reject_request(
        db, company_id, request_id, current_user.id, payload.note
    )
    emp_names = await _employee_names(db, company_id)
    types = await _type_map(db, company_id)
