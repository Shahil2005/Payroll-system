...
from app.constants import DEFAULT_FINANCIAL_YEAR, PayrollCycleStatus, Permission
...
from app.models.payroll import Payslip, PayrollCycle
from app.models.user import User
from app.schema.leave import (
    LeaveBalanceOut,
    LeaveDecisionIn,
    LeaveRequestIn,
    LeaveRequestOut,
...
from app.schema.payroll import MyPayslipOut
...
from app.services import leave_service, timesheet_service
...
async def my_timesheet_detail(
    ...
    ts = await timesheet_service.get_owned_detail(
        db, timesheet_id, company_id, employee_id
    )
...
async def my_leave_balances(
    ...
    rows = await leave_service.list_balances(
        db, company_id, DEFAULT_FINANCIAL_YEAR, employee_id=employee_id
    )
...
async def cancel_my_leave_request(
    ...
    req = await leave_service.cancel_request(
        db, company_id, request_id, actor_id, payload.note
    )
