...
from app.constants import TimesheetMode, TimesheetStatus
...
class Timesheet(Base, TimestampMixin):
    ...
    mode: Mapped[str] = mapped_column(
        String(16),
        default=TimesheetMode.ATTENDANCE.value,
        server_default=TimesheetMode.ATTENDANCE.value,
    )
    ...
    half_days: Mapped[Decimal] = mapped_column(
        Numeric(6, 2), default=Decimal("0"), server_default=text("0")
    )
    ...
    __table_args__ = (
        UniqueConstraint("cycle_id", "employee_id", name="uq_timesheet_cycle_employee"),
    )
